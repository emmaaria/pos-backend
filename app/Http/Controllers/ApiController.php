    ...
    */
    ...
    public function getProductsWithStock(Request $request)
    {
        ...
        if ($companyId) {
            ...
            if (empty($name)) {
                $products = DB::table('products')
                    ->select('products.name AS product_name', 'products.product_id AS product_id', 'products.price AS price', DB::raw('SUM(SUM(purchase_items.quantity) - SUM(invoice_items.quantity)) as stock'))
                    ->where('products.company_id', $companyId)
